...
async function createTables() {
  ...
  try {
    ...
    console.log('Creating EcoGreen Sales Orders tables...');

    // 1. ecogreen_sales_orders Table
    await client.query(`
      CREATE TABLE IF NOT EXISTS ecogreen_sales_orders (
    ...
    // 2. ecogreen_sales_order_items Table
    await client.query(`
      CREATE TABLE IF NOT EXISTS ecogreen_sales_order_items (
        id SERIAL PRIMARY KEY,
        sales_order_id INTEGER REFERENCES ecogreen_sales_orders(id) ON DELETE CASCADE,
        item_seq INTEGER,
