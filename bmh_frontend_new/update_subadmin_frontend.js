...
const path = 'c:/Users/Lohitha Asish/Desktop/BMH/bmh_frontend_new/app/department/dashboard/doctors/index.tsx';
let content = fs.readFileSync(path, 'utf8');

// 1. Add Cancelled Tokens to TABS
content = content.replace(
  "const TABS = ['Doctors', 'Slots', 'Bookings'];",
  "const TABS = ['Doctors', 'Slots', 'Bookings', 'Cancelled Tokens'];"
);
...
const newState = `
  ...
  const handleProcessRefund = async () => {
    ...
    try {
      const userData = await AsyncStorage.getItem('departmentUser');
      let userId = '1';
      if (userData) {
...
`;
// find where to inject it. Before `</View>\n        </ScrollView>\n      )}`
content = content.replace(
  "          </View>\n        </ScrollView>\n      )}",
  cancelledTokensUI + "\n          </View>\n        </ScrollView>\n      )}"
);
...
console.log('Successfully updated index.tsx (Department/Subadmin Doctors)');
