...
export default function AdminTasksScreen() {
  ...
  const fetchRecurringTasks = async () => {
    try {
      const res = await axios.get('https://napi.bharatmedicalhallplus.com/tasks/recurring', {
        headers: { Authorization: `Bearer ${localStorage.getItem('superAdminToken')}` } 
      });
      // The backend uses req.user, but wait, does super admin send token? 
      // The current fetchTasks uses `?user_type=super_admin`. Let me check if recurring supports this, or I should update backend to allow `?user_type=super_admin` without token.
    } catch(e) {}
  };
