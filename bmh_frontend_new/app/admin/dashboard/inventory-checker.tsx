...
export default function AdminInventoryChecker() {
  ...
  const [activeTab, setActiveTab] = useState<'tasks' | 'pending' | 'history' | 'daily'>('tasks');
  ...
  const handleReviewMismatch = async (id: number, status: 'approved' | 'rejected', assigneeId?: string, priority?: string, dueDate?: string) => {
    setReviewing(id);
    try {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      <View style={styles.header}>
        <Text style={styles.title}>Inventory Checker Management</Text>
        <Text style={styles.subtitle}>Assign inventory tasks to staff and approve or reject mismatches.</Text>
      </View>
                  ...
                  .map(r => {
                    ...
                    return (
                      <TouchableOpacity 
                        ...
                        onPress={() => {
                          setSelectedRacks([...selectedRacks, rackName]);
                          setRackSearch('');
              ...
              <TouchableOpacity 
                ...
                onPress={() => {
                  ...
                  if (pendingItemId !== null) {
                    handleReviewMismatch(pendingItemId, 'approved', selectedAssigneeId, selectedPriority, selectedDueDate);
                  }
                }}
              >
