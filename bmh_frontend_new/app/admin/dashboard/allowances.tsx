...
export default function AdminAllowancesScreen() {
  ...
  const handleAllocate = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setAllocAmount(''); setAllocNote(''); setSelectedEmployeeId(''); setSelectedDepartmentId('');
  ...
  const filteredHistoryTxs = transactions.filter(t => {
    ...
    if (historyFilterDept && t.department !== historyFilterDept) return false;
  ...
  const handleExportHistoryCSV = () => {
    ...
    if (Platform.OS === 'web') {
      ...
      const url = URL.createObjectURL(blob);
      link.setAttribute('href', url);
      link.setAttribute('download', `transaction_history_export.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
          ...
          <View style={[styles.modalContent, isDesktop && { width: 500 }]}>
            ...
            <ScrollView style={{ maxHeight: 150, borderWidth: 1, borderColor: Colors.light.border, borderRadius: 8, marginBottom: 20 }}>
              {departments.map(d => (
                <Pressable 
                  key={d.id} 
                  style={[styles.pickerItem, selectedDepartmentId === d.id.toString() && styles.pickerItemActive]}
                  onPress={() => {
                    setSelectedDepartmentId(d.id.toString());
                    setSelectedEmployeeId(''); // Reset employee when department changes
                  }}
                >
                  <Text style={[styles.pickerItemText, selectedDepartmentId === d.id.toString() && styles.pickerItemTextActive]}>
                    {d.name}
                  </Text>
                </Pressable>
              ))}
            ...
            <ScrollView style={{ maxHeight: 150, borderWidth: 1, borderColor: Colors.light.border, borderRadius: 8, marginBottom: 20 }}>
              {!selectedDepartmentId ? (
                <Text style={{ padding: 16, color: Colors.light.icon, fontStyle: 'italic' }}>Please select a department first.</Text>
              ) : allEmployees.filter(e => {
                const selectedDept = departments.find(d => d.id.toString() === selectedDepartmentId);
                return selectedDept ? e.department === selectedDept.name : false;
              }).length === 0 ? (
                <Text style={{ padding: 16, color: Colors.light.icon, fontStyle: 'italic' }}>No employees found in this department.</Text>
              ) : allEmployees.filter(e => {
                const selectedDept = departments.find(d => d.id.toString() === selectedDepartmentId);
                return selectedDept ? e.department === selectedDept.name : false;
              }).map(e => (
            ...
            <View style={styles.modalActions}>
              <Pressable style={styles.cancelBtn} onPress={() => { setAllocationModalVisible(false); setAllocAmount(''); setAllocNote(''); setSelectedEmployeeId(''); setSelectedDepartmentId(''); setAllocPaymentMode('Cash'); setAllocTxnId(''); }}><Text style={styles.cancelBtnText}>Cancel</Text></Pressable>
