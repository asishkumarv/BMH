              ...
              registerForPushNotificationsAsync().then((token) => {
                if (token && token !== u.push_token) {
                  axios.post('https://napi.bharatmedicalhallplus.com/employees/update-push-token', {
                    ...
                    console.log("Refreshed push token on server:", token);
                    u.push_token = token;
                    AsyncStorage.setItem('employeeUser', JSON.stringify(u)).catch(err => {
                      console.log("Error saving updated user data:", err);
                    });
                  }).catch(err => {
                    console.log("Error updating push token on server:", err?.response?.data || err.message);
                  });
                }
              }).catch(err => {
                console.log("Error during push token auto-refresh:", err);
              });
            }
          }
        }
    ...
    <SafeAreaView style={styles.safeArea}>
        ...
        <Modal transparent visible={isMobileSidebarOpen} animationType="fade">
          <View style={styles.overlay}>
            ...
            <View style={styles.drawerContainer}>
              <DeliverySidebar onClose={() => setIsMobileSidebarOpen(false)} />
            </View>
          </View>
        </Modal>
      )}
      ...
      <View style={styles.container}>
        ...
        <View style={[
          ...
          <TopHeader 
            userType="delivery" 
            title={!isDesktop ? "Delivery Portal" : undefined}
