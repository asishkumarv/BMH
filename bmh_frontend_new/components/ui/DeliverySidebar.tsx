...
const NAV_ITEMS = [
  ...
  { name: 'Performance', icon: LayoutDashboard, route: '/delivery/dashboard/performance' },
  { name: 'Profile', icon: User, route: '/delivery/dashboard/profile' },
];

export const DeliverySidebar = ({ onClose }: { onClose?: () => void }) => {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.logoContainer}>
        ...
        <Text style={styles.logoText}>Delivery Portal</Text>
      </View>

      <ScrollView style={styles.navContainer} showsVerticalScrollIndicator={false}>
        {NAV_ITEMS.map((item) => {
          const isActive = pathname === item.route || (item.route !== '/delivery/dashboard' && pathname.startsWith(item.route));
